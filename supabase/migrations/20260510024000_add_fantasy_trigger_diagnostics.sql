-- Diagnostic instrumentation for fantasy heartbeat trigger path.
-- Temporary logging to inspect matching and update flow in Supabase/Postgres logs.
...
begin
  if p_player_id is null or p_player_id <= 0 then
    raise notice '[fantasy_recalc] skip invalid player_id=%', p_player_id;
    return;
  end if;
  ...
  raise notice '[fantasy_recalc] player_id=% affected_entries=% updated_entries=%', p_player_id, affected_count, updated_count;
end;
$$;

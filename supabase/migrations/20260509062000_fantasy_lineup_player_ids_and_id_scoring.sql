-- Backfill lineup JSON from ["Name"] to [{"player_id":123,"player_name":"Name"}] when possible,
-- then switch fantasy recompute trigger logic to strict player_id matching.
  ...
  select
    ...
    case
      when jsonb_typeof(elem.value) = 'object' then
        case
          when coalesce(nullif(elem.value->>'player_name', ''), nullif(elem.value->>'playerName', '')) is null then null
            ...
            'player_name',
              coalesce(nullif(elem.value->>'player_name', ''), nullif(elem.value->>'playerName', ''))
          )
        end

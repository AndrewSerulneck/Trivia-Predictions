...
import type {
  ScategoriesSession,
  ScategoriesRound,
  ScategoriesSubmission,
  ScategoriesCategoryResult,
  ScategoriesRoundResults,
} from "@/types";
import {
  listAllActiveSchedules,
  isWindowOpen,
  nextOccurrence,
} from "@/lib/scategoriesSchedules";
import categorySetsData from "@/data/scategories/category-sets.json";
...
export function scategoriesChannelName(venueId: string): string {
  return `scategories-session:${venueId}`;
}

function broadcast(venueId: string, event: string, payload: Record<string, unknown>): void {
  if (!supabaseAdmin) return;
  void supabaseAdmin.channel(scategoriesChannelName(venueId)).send({
    type: "broadcast",
    event,
    payload,
  });
...
function toSession(row: SessionRow): ScategoriesSession {
  return {
    id: row.id,
    venueId: row.venue_id,
    status: row.status as ScategoriesSession["status"],
...
function toRound(row: RoundRow): ScategoriesRound {
  return {
    id: row.id,
    ...
    endsAt: row.ends_at,
    status: row.status as ScategoriesRound["status"],
    createdAt: row.created_at,
  };
}

function toSubmission(row: SubmissionRow): ScategoriesSubmission {
  return {
    id: row.id,
...
export async function getActiveSession(venueId: string): Promise<ScategoriesSession | null> {
  assertAdmin();
  const { data, error } = await supabaseAdmin!
    .from("scategories_sessions")
    .select(SESSION_COLS)
    .eq("venue_id", venueId)
    .in("status", ["lobby", "active", "scoring"])
    .maybeSingle<SessionRow>();

  if (error) throw new Error(error.message || "Failed to load Scategories session.");
  return data ? toSession(data) : null;
}

export type CreateSessionOptions = {
  /** 'auto' for engine-driven sessions, 'manual' for admin-driven. Defaults to 'manual'. */
  source?: ScategoriesSession["source"];
...
export async function createSession(
  ...
  if (error) {
    ...
    throw new Error(error.message || "Failed to create Scategories session.");
  }
  return toSession(data);
}

/** Advance a lobby session to 'active' and create the first round. */
export async function startRound(sessionId: string): Promise<ScategoriesRound> {
  assertAdmin();
 ...
 */
export async function scoreRound(roundId: string): Promise<ScategoriesRoundResults> {
  assertAdmin();
  ...
  await Promise.all(
    Array.from(pointsByUser.entries()).map(([userId, pts]) =>
      pts > 0 ? awardScategoriesPoints({ userId, venueId: round.venue_id, points: pts }) : Promise.resolve()
    )
  );
...
async function awardScategoriesPoints(params: {
  userId: string;
  venueId: string;
  points: number;
}): Promise<void> {
  ...
  const campaignResult = await applyChallengeCampaignPoints({
    userId,
    venueId,
    gameType: "live-trivia", // closest existing type; scategories will get its own when ChallengeGameType expands
    basePoints: points,
  }).catch(() => null);
...
async function buildResults(roundId: string, round: RoundRow): Promise<ScategoriesRoundResults> {
  ...
  const results: ScategoriesCategoryResult[] = categories.map((category, idx) => {
...
export async function getRoundResults(roundId: string): Promise<ScategoriesRoundResults> {
  assertAdmin();
 ...
 * Called by the cron job at /api/cron/scategories-score.
 */
...
export type ScategoriesEngineResult = {
...
/**
 * Drives every venue's S'Categories experience with no human in the loop:
 ...
 */
export async function runScategoriesEngine(now: Date = new Date()): Promise<ScategoriesEngineResult> {
  assertAdmin();

  const result: ScategoriesEngineResult = {

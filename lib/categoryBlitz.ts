...
/**
 * Idempotently record that a user's client observed live state for a session.
 * Repeat calls (refreshes, re-polls) are no-ops. The participant row is what
 * drives `playerCount` (used by the uniqueness-scoring "insufficient_players"
 * override and the invite banner) — Category Blitz runs as a continuous loop,
 * so there is no player-vs-spectator distinction; anyone present can play the
 * live round immediately.
 */

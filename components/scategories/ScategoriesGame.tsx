...
import { getVenueId, getUsername } from "@/lib/storage";
import { useScategoriesSession } from "@/lib/scategoriesRealtime";
import type { ScategoriesRoundResults } from "@/types";
...
function IdleScreen({ venueId }: { venueId: string | null }) {
  ...
  useEffect(() => {
    if (!venueId) return;
    void fetch(`/api/scategories/sessions?venueId=${encodeURIComponent(venueId)}`)
      .then((r) => r.json())
  ...
  const countdownSeconds = nextWindowAtMs != null ? Math.max(0, Math.floor((nextWindowAtMs - nowMs) / 1000)) : null;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 px-4 py-8">
      <div className={`w-full max-w-sm rounded-2xl border ${BORDER_CARD} bg-slate-900/70 p-6 text-center`}>
        <p className={TEXT_LABEL}>S&apos;Categories</p>
...
function ResultsScreen({ results, userId }: { results: ScategoriesRoundResults; userId: string }) {
    ...
    try {
      ...
      await Promise.all(
        filled.map(({ categoryIndex, answer }) =>
          fetch(`/api/scategories/rounds/${roundId}/submit`, {
            method: "POST",
...
export function ScategoriesGame() {
  const venueId = getVenueId() ?? "";
  const username = getUsername() ?? null;
  const userId = useRef<string>("");

  useEffect(() => {
    // Resolve userId once client-side (from local storage via session cookie lookup)
    import("@/lib/storage").then(({ getUserId }) => {
      userId.current = getUserId() ?? "";
    });
  }, []);

  const { phase, round, results, timeRemaining, error } = useScategoriesSession(venueId);
  ...
  return (
    ...
    >
      ...
      <div className={`shrink-0 border-b ${BORDER_ACTIVE} bg-slate-950 px-4 py-3`}>
        <div className="flex items-center gap-2">
          ...
          <p className={`text-[0.7rem] font-black uppercase tracking-[0.16em] ${TEXT_ACCENT}`}>
            {phase === "idle" ? "S’Categories" : phase === "lobby" ? "Lobby" : phase === "answering" ? "Round Active" : phase === "scoring" ? "Scoring" : phase === "results" ? "Results" : "Game Over"}
          </p>
        </div>
      </div>
      ...
      {phase === "results" && results && (
        <ResultsScreen results={results} userId={userId.current} />
      )}

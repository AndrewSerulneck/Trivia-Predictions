...
export type SportsBingoSelectBoardProps = {
  ...
  /** Sheet host: return to game selection if this game stops being eligible. */
...
export function SportsBingoSelectBoard({
  ...
  const loadGame = useCallback(async () => {
    ...
    const cachedGame = readSelectedBingoGame({ sportKey, gameId, tzOffsetMinutes });
    if (cachedGame) {
      setGame(cachedGame);
    }
    ...
    try {
      const response = await fetch(
        `/api/bingo/games?sportKey=${encodeURIComponent(sportKey)}&includeLocked=false&tzOffsetMinutes=${encodeURIComponent(String(tzOffsetMinutes))}`,
        { cache: "no-store" }
      );
      ...
      setGame(selected);
      if (!selected) {
        clearSelectedBingoGame({ sportKey, gameId });
        if (onGameUnavailable) {
          onGameUnavailable();
        } else {
          router.replace(`/bingo/select-game?sportKey=${encodeURIComponent(sportKey)}`);
        }
      } else if (payload.incomplete && payload.warning) {
  ...
  }, [gameId, onGameUnavailable, router, sportKey]);
  ...
  useEffect(() => {
    ...
    const revalidate = () => {
      if (document.visibilityState !== "hidden") void loadGame();
    };

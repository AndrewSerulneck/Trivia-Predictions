  ...
  return (
    <div className="relative flex h-28 w-28 items-center justify-center">
      <motion.div
        ...
        animate={reducedMotion ? { scale: 1, opacity: 1 } : { scale: [0.92, 1.08, 0.92], opacity: [0.5, 0.95, 0.5] }}
        transition={reducedMotion ? { duration: 0 } : { duration: 2.8, ease: "easeInOut", repeat: Number.POSITIVE_INFINITY }}
      />
      <motion.div
        aria-hidden
        className="absolute inset-[10%] rounded-full border border-amber-300/35"
        animate={reducedMotion ? { scale: 1, opacity: 1 } : { scale: [1, 1.14, 1], opacity: [0.22, 0.62, 0.22] }}
        transition={reducedMotion ? { duration: 0 } : { duration: 2.2, ease: "easeInOut", repeat: Number.POSITIVE_INFINITY, delay: 0.2 }}
      />
  ...
  return (
    <AnimatePresence>
      {content ? (
        ...
        >
          <motion.div
            initial={reducedMotion ? false : { y: 24, opacity: 0, scale: 0.97 }}
            animate={{ y: 0, opacity: 1, scale: 1 }}
            exit={reducedMotion ? { opacity: 0 } : { y: 20, opacity: 0, scale: 0.98 }}
            ...
            <div className="relative flex flex-col items-center text-center">
              ...
              <div className="mt-6 grid w-full gap-3">
                <button
                  ...
                  disabled={isBusy}
                  aria-busy={isBusy}
                  className="tp-player-hit-target tp-player-pressable inline-flex min-h-[54px] w-full items-center justify-center gap-2 rounded-full bg-[linear-gradient(135deg,#fde68a,#f59e0b)] px-4 py-3 text-sm font-black text-slate-950 transition-transform duration-200  disabled:opacity-70"
                >
                  {isBusy ? <RefreshCw className="h-4 w-4 animate-spin motion-reduce:animate-none" strokeWidth={2.5} /> : null}
                  {isBusy ? "Checking…" : content.primaryLabel}
                </button>
                ...
                {showSecondaryAction && content.secondaryLabel && onSecondaryAction ? (
                  <button
                    ...
                    className="tp-player-hit-target tp-player-pressable inline-flex min-h-[52px] w-full items-center justify-center rounded-full border border-white/14 bg-white/[0.05] px-4 py-3 text-sm font-black text-white/92 transition-colors duration-200 hover:bg-white/[0.08]"
                  >

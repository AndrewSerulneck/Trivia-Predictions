...
- **`/info` IS the home page.** It is the marketing site we want Google to index and the page a first-time visitor sees. **Every internal control that means "go to the home page" — a Back button, a nav logo, a marketing link — must target `/info`, never `/`.** The apex DNS flip is not done yet: until `NEXT_PUBLIC_DOMAIN_SPLIT_ENABLED` is on, `hightopchallenge.com/` still serves the player game login (`/` → `JoinFlow`), so a "home" link pointed at `/` silently strands partners and first-time visitors on a *player sign-in* screen. Build home links with `marketingHref("/info")` (`lib/domainSplit.ts`) so they stay relative today and become absolute apex URLs after the split. Regression fixed 2026-09-05: `/owner/login`'s "Back to Home Page" and the `/info` nav logo both pointed at `/`.
- **The player game login relocates to `play.hightopchallenge.com`.** (This is the change that finally frees apex `/`.) The current apex login (`/` → `JoinFlow`) and all venue/game routes move under the `play.` subdomain. Apex host → marketing (`/info`); `play.` host → the game. This is enforced at the edge in **`proxy.ts`** (the live Next.js 16 edge gate — the framework renamed `middleware.ts` → `proxy.ts`; do **not** add a `middleware.ts`, it's a build error) via `lib/domainSplit.ts`, **shipped behind `NEXT_PUBLIC_DOMAIN_SPLIT_ENABLED` (off = today's single origin, fully inert).** To flip it live without downtime, follow the exact runbook: **`docs/phase-6-domain-split-runbook.md`** (DNS + envs + smoke tests; reversal is one flag). The split layers in front of `proxy.ts`'s existing (live) cookie auth-gate without changing it.
- **The old "payments page" is becoming the mobile-first Partner Dashboard.** The `/owner/*` surface (currently owner auth + subscription/billing, reached via the "Partner Login" button on `/info`) is being expanded into a mobile-first **Partner Dashboard** for subscriber venues. A subscriber venue is a venue that pays for our geofenced platform so their guests can play. From their phone, a partner will be able to:
  ...
  3. **Submit payment / manage subscription.** Billing runs on **Stripe** (`lib/stripe.ts` + the webhook at `/api/webhooks/stripe`); the Partner Dashboard is the surface for subscribe / update card / invoices.
- **Partner self-serve signup ends the admin-activation prerequisite.** A partner can subscribe end-to-end without an admin pre-creating their venue: the full-screen wizard at `/owner/signup` collects name → email → password → address → geofence → review, creates a hidden `venues` row + owner account, and hands off to Stripe Checkout; the webhook unhides the venue on first paid activation. Flag-gated behind `NEXT_PUBLIC_SELF_SERVE_SIGNUP_ENABLED` (`isSelfServeSignupEnabled()` in `lib/selfServeSignup.ts`) — off = `/owner/register`'s venue-lookup flow, fully inert; on = `/owner/register` redirects to `/owner/signup` and the `/info` partner CTAs point there. **Admin Activate-a-Venue (`components/admin/mobile/ActivateVenueFlow.tsx`) survives** for ops: offline-billed venues, a second venue for an existing owner (self-serve is one venue per email), and support fixes. Plan + as-built: `docs/partner-self-serve-signup-plan.md`; ops cutover + the `GOOGLE_MAPS_API_KEY` referrer restriction: `docs/self-serve-signup-runbook.md`. **Status (2026-09-08):** built, `/code-review`'d, and all 15 findings fixed across `docs/self-serve-signup-review-fixes-plan.md` Phases 1–7 (claim-path privilege escalation, atomic Postgres rate limiter, fail-closed public Maps key, reveal reconciler, and the "delete the second copy" cleanups). The flag is **not yet flipped** — what gates it is Phase 8's ops sequence: set `GOOGLE_MAPS_BROWSER_KEY`, re-verify admin + player Maps, re-run `/code-review`, redeploy, then close the device checklist.
...
### Navigation controls (unified 2026-09-05 — `docs/navigation-unification-plan.md`)
Before this, Back had 10 implementations across 5 visual treatments, Next had 2, and Sign Out had 5 partial implementations that never fully tore down a session. Phases 0-7 collapsed all of it into four primitives in `components/navigation/`. **Do not add a fifth, and do not re-implement their behavior inline.**

- **`ExitBackButton`** - THE Back ("leave this screen for its parent"). A neutral dark-slate 34px circle with a Lucide `ChevronLeft`; exactly one per screen, pinned top-left in the sticky header. Never inline in content, never at the bottom of a screen, never in a wizard footer. `tone="light"` is the same button/position/behavior on the admin and `/owner/*` white-card surfaces.
- **`StepBackButton` + `NextButton`, composed by `WizardFooter`** - step progression inside a multi-step flow (JoinFlow, `CreateRewardWizard`, `/owner/register`, `ActivateVenueFlow`). These two are rendered only by `WizardFooter.tsx`.
- **`SignOutButton`** - "leave your account." `variant: "player" | "partner" | "admin"`, each wired to a real teardown. Lives as the LAST item of an account drawer / sidebar footer, below a divider, so it can never sit adjacent to Back; it never renders an arrow.
- **`components/navigation/exitNavigation.ts`** holds Back's *behavior*, separate from its markup: `useExitNavigation` owns the precedence order (`onExit` -> `venueHomeFallback` -> history), the installed-PWA hardening (`history.length <= 1` -> internal referrer -> `href`), the 150ms post-`history.back()` fallthrough, `triggerBackHaptic()`, and the venue-game return transition (`runVenueGameReturnTransition` / `navigateBackToVenue`).
- **Host shells own the Back slot, not the pages.** Player content pages pass `PageShell backTo={...}`; `/owner/*` pages pass `OwnerShell backTo={...}`; the four `GameAppBar` games inherit it from `components/venue/AppBar.tsx`'s defaulted `leading` slot. `components/navigation/AccountMenuList.tsx` is the one shared account-drawer list behind both `AccountMenu` and `VenueHubClient`'s drawer.
- **Player sign-out is now complete.** `POST /api/join/logout` (added in Phase 3) is the only thing that can expire the HttpOnly 90-day `tp_sess` cookie; `performSignOut("player")` runs it (`keepalive`, capped wait) then `hardClearAuthAndCache()` then Supabase `signOut()`. "Leave Venue" on venue home was renamed **Sign Out** and actually signs out.
- **Deleted, do not resurrect:** `components/navigation/BackButton.tsx`, `components/predictions/BackToVenueButton.tsx`, the extensionless `components/animations/LiveTriviaIntermissionLeaderboardReveal`, `.tp-exit-pill` / `.ht-btn-exit` / the `--ht-exit-*` vars in `app/globals.css`, and the `ht.color.exit` scale in `tailwind.config.ts`.
- **Tripwire:** `tests/navigation-controls-contract.test.ts` (7 static assertions), runs under plain `npm run test`.
...
- **Navigation (Phase 6 of the nav unification):** `AdminShell.tsx` / `AdminMobileShell.tsx` and the 3 "← Back to list" section links (`ChallengesSection`, `SchedulesSection` ×2) use `SignOutButton variant="admin"` and `ExitBackButton tone="light"`. Admin is deliberately a light-themed, non-player surface: it adopts the **positions and behavior** of the unified controls but not the dark-native circle styling. One bespoke light exit button remains by choice — `components/admin/mobile/ActivateVenueFlow.tsx` (optional cleanup, `docs/navigation-unification-plan.md` §13e).
...
- **Auth:** `venue_owners` + `venue_owner_venues` tables; guarded by `lib/requireOwnerAuth.ts`. Pages: `/owner/login`, `/owner/register`, `/owner/signup`, `/owner/forgot-password`, `/owner/reset-password`, `/owner/dashboard`, `/owner/billing`, `/owner/billing/setup`. Shared shell: `components/owner/OwnerShell.tsx` — **except `/owner/signup`**, which is a full-bleed takeover on its own `components/signup/SignupShell.tsx` (progress rail + `ExitBackButton` + `WizardFooter`, no OwnerShell card) and is listed in `AppShell`'s `FULLSCREEN_PATHS`. `/owner/register` is a server component that redirects to `/owner/signup` when `NEXT_PUBLIC_SELF_SERVE_SIGNUP_ENABLED` is on, else renders the legacy `components/owner/OwnerRegisterForm.tsx`.
- **Navigation (Phase 5 of the nav unification):** `OwnerShell` owns the header row — a `backTo` slot rendering `ExitBackButton` and a `showAccountMenu` flag rendering `OwnerAccountMenu` (whose drawer holds `SignOutButton variant="partner"`). This replaced 7 hand-rolled "← Dashboard" links and 5 bare text links, and made Sign Out reachable from all 12 `/owner/*` pages instead of only the dashboard. Pages must not add their own back link. The dashboard is the root screen and deliberately has no `backTo`; every other authenticated page backs to `/owner/dashboard`, and the pre-auth pages (`login` / `register` / `forgot-password` / `reset-password`) carry no account menu. **`/owner/login`'s "Back to Home Page" targets `marketingHref("/info")`** — see §0; pointing it at `/` sends partners to the *player* sign-in.
...
- **Billing (Stripe only):** `app/api/owner/billing/*` (`route`, `subscription`, `checkout`, `portal`) + the signature-verified webhook at `app/api/webhooks/stripe`, which is the source of truth for status. `billing_subscriptions` mirrors Stripe (`stripe_customer_id`, `stripe_subscription_id`, status, period, plus the additive `discount_*` mirror). `billing_method` splits card-billed (`stripe`) from admin-granted check payers (`offline`); offline rows carry no Stripe ids and are expired by `/api/cron/billing`. That daily cron now has **three** jobs (`lib/venueVisibilitySync.ts`): offline-grant expiry; `repairMissedVenueReveals`, which reveals any self-serve venue that is paying but still `hidden` because the webhook follower missed it; and `reconcileLapsedVenues`, which restores a venue we hid (billing live again), re-hides a lapsed self-serve venue, and reports (count-only) lapsed admin-activated venues. The restore + re-hide jobs are gated on `VENUE_REHIDE_ENABLED` and ship log-only. `?dryRun=1` forces report-only for both reconcilers; a reconciler fault never fails the cron. See §12 and CLAUDE.md's "Venue Visibility" section. The pre-launch **SlimCD** processor was removed entirely on 2026-08-01 (module, session/return/card/subscribe routes, and the cron charge loop); its two columns survive as dead weight and nothing reads them — `tests/billing-slimcd-removed.test.ts` is the tripwire.
- **A `billing_subscriptions` row means a partner who paid; an unfinished signup leaves nothing behind.** The webhook only ever *creates* a row (never gates *updates*) when Stripe reports the subscription as paid-for (`active`/`trialing`) — a Checkout session that completes unpaid, or an `incomplete` subscription with no existing row, writes nothing. This makes `past_due` unambiguous: it means an established subscriber's card failed, never an abandoned first attempt. See `docs/billing-code-review-fixes-plan.md` Phase 8 and its run-log write-up before touching webhook row-creation logic — it's easy to "helpfully" persist an in-progress checkout and reintroduce the ambiguity.
...
- Color philosophy:
  ...
  - **Exit/back actions are NEUTRAL, not warm.** The old warm red-orange exit gradient (`--ht-exit-*` / `.tp-exit-pill` / `.ht-btn-exit` / Tailwind `ht.color.exit`) was retired and deleted on 2026-09-05. Back is the dark-slate `ExitBackButton` circle (`border-white/10 bg-slate-900`, Lucide `ChevronLeft`), and it is **never tinted per game** — that neutrality is what lets the identical button sit unchanged on Bingo's felt, Category Blitz's emerald and Live Trivia's cyan. The `--ht-exit-*` tokens survive only in `design-system/**/colors_and_type.css` to keep archived HTML preview specimens rendering; nothing in `app/` or `components/` may use them.
...
- Venue visibility contract (see CLAUDE.md's "Venue Visibility" section):
  - Lifecycle: a self-serve signup creates the `venues` row `hidden = true` → the Stripe webhook (`maybeRevealVenue`) unhides it on the first paid sync → the daily `/api/cron/billing` reconciler (`repairMissedVenueReveals`) repairs that if the webhook missed → when billing goes not-live, `maybeRehideVenue` (webhook) and `reconcileLapsedVenues` (cron) re-hide the venue and stamp `venues.rehidden_at`, and restore it on resubscribe. Re-hide/restore are gated on `VENUE_REHIDE_ENABLED` (server-side, no `NEXT_PUBLIC_`) and ship LOG-ONLY — flag off = they run in dry-run and write nothing; the cron's admin-lapse **report** job always runs. Every un-hide path clears `rehidden_at`.
  - `venues.hidden` is soft (join-list exclusion only, in `listVenues()`), never a hard access gate.
  - `lib/venueVisibility.ts` is the one home of `shouldRevealVenue` / `shouldRehideVenue` / `shouldRestoreVenue`, all on `classifyBillingRow()`. Never compute `current_period_end < now` in a visibility path; never delete a `billing_subscriptions` row on cancellation.
  - A visibility bug is fixed in the reconciler, never by making the webhook follower throw.
  - Admin visibility: the "Hidden venues" panel at the bottom of the admin Venues section (`HiddenVenuesPanel`, `GET /api/admin?resource=hidden-venues`) lists every hidden venue badged lapsed / admin-hidden / system-room; lapsed rows get a **Restore (unhide)** (`POST resource=venue-visibility`, `restoreLapsedVenue`) that clears `rehidden_at` but is billing-agnostic — the reconciler re-hides it next run if billing is still not live.
  - A venue at `(0,0)` (both Category Blitz global rooms) is never claimable, joinable, or revealable — `isPlaceholderVenueRow` in `lib/venueClaim.ts`.
- Navigation contract (see the "Navigation controls" subsection under §3 and `docs/navigation-unification-plan.md`):
  - Back, step-Next and Sign Out have exactly one implementation each (`ExitBackButton`, `WizardFooter`, `SignOutButton`). Never hand-roll another, and never re-implement `useExitNavigation`'s history/PWA logic at a call site.
  - Back is pinned top-left in the host shell's header, one per screen; Sign Out is the last item of an account drawer and never renders an arrow.
  - `signOut()`, `clearVenueSession()` and the three logout endpoints are called from `SignOutButton.tsx` only. The two allow-listed non-sign-out exceptions are `JoinFlow` (drops a stale Supabase session mid-login) and `VenueHubClient` (arrival watchdog).
  - The warm exit-pill styling is retired; `tests/navigation-controls-contract.test.ts` fails the build on any of the retired literals, a raw `←` in player-facing TSX, or a control rendered outside its sanctioned host.
  - "Home page" means `/info`. A home link pointed at `/` is a bug until the domain split flips (§0).

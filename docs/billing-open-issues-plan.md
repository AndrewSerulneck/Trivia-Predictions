...
> billing-scoped.
>
> **Status as of 2026-08-02 (code-review-round4-plan.md Phase 8 close-out):**
> Items **Z, AA** (new, from the fourth `/code-review` pass) are closed — see
> the "Closed" column and `docs/code-review-round4-plan.md`. That round's other
> five findings (NFL Pick 'Em spread double-count, scoring-mode-read
> degradation, and the line-refresh lock race) are recorded in
> `docs/nfl-pickem-spread-line-settlement-locking-fix-plan.md`'s "Round 4 Code
> Review Findings" section — not in this inventory table, which is
> billing-scoped.
...
| Z | `syncDiscountFromEvent`'s non-`removed` branch built a full mirror from `discountMirrorFromStripe(discount, await resolveDiscountCoupon(discount))` even when the coupon retrieve failed — a transient Stripe outage produced a valueless-but-written mirror (`discount_percent_off`/`discount_amount_off_cents` both null, label falling back to the raw coupon id), showing list price on the partner's page while Stripe kept applying the real discount | fourth `/code-review` pass, finding 6 | ✅ closed — `code-review-round4-plan.md` Phase 6: added `DiscountCouponResolution` (`absent`\|`resolved`\|`unresolved`) + `resolveDiscountCouponResult` in `lib/billingDiscounts.ts`, keeping `resolveDiscountCoupon` as a thin wrapper so `resolveDiscountMirror`'s two call sites are untouched; `syncDiscountFromEvent` now skips the write and warns (naming the subscription + coupon ref) only on `unresolved`, matching `resolveDiscountMirror`'s existing "Stripe unreachable — keep the existing mirror" contract. `absent` (no coupon ref) and expanded-coupon payloads still write exactly as before. Noted but explicitly **not** fixed in this round: `resolveDiscountMirror` has the same latent defect on its own two inner coupon resolutions — worth its own inventory item if it resurfaces. Mocked-SDK tests only (`tests/api.webhooks.stripe-discount-sync.test.ts`) |
| AA | `grant-manual` mutated Stripe twice (force-cancel, then discount detach) before ever validating `paidThroughDate` — a malformed or past date still 400'd, but only after the partner's discount was irreversibly detached and, on `force: true`, their subscription scheduled for cancellation; nothing rolled back | fourth `/code-review` pass, finding 7 | ✅ closed — `code-review-round4-plan.md` Phase 7: hoisted the `paidThroughDate` format/future-date validation to immediately after the `existingSub` select and before the force-cancel branch, so both Stripe mutations now sit below all validation; added an invariant comment marking the line below which nothing may 400. The 409 guards, the detach's skip conditions, and `removalIsMootAtStripe` were untouched — only the ordering moved. Mocked-SDK tests only (`tests/api.admin.billing-grant-manual-validates-first.test.ts`, new) |

...
Not needed for this phase. There is no application code. The two migrations were verified
against production, which is stronger evidence than a read-through:

- The baseline was generated by script from the live ACLs, then shown to reproduce them
  exactly on a replay (138 of 138 items, none missing or extra).
- Production's ACLs are byte-identical before and after.
- A probe proved the new default is live.

If Andrew wants a second opinion anyway, the only judgement call is the `revoke all`
deviation (§3).

...
export async function POST(request: Request) {
  try {
    ...
    if (uploaded.size > MAX_IMAGE_BYTES) {
      return NextResponse.json({ ok: false, error: "File must be under 1MB." }, { status: 400 });
    }

    const extension = getFileExtension(uploaded.type, uploaded.name);
    const today = new Date().toISOString().slice(0, 10);
    const objectPath = `challenge-assets/${today}/${crypto.randomUUID()}.${extension}`;
    ...
    if (uploadError) {
      throw new Error(uploadError.message || "Failed to upload challenge image.");
    }
    ...
    if (!publicUrl) {
      throw new Error("Failed to generate challenge image URL.");
    }
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Failed to upload challenge image." },
      { status: 500 }
    );
  }
}
